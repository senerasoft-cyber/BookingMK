import { Check, Image as ImageIcon, Images, Palette, Trash2, Type } from 'lucide-react'
  ...
  return (
    <section className="rounded-2xl border border-stone-200 p-5 dark:border-stone-800">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="font-display text-sm font-semibold text-stone-900 dark:text-stone-50">{title}</h2>
      </div>
      {children}
    </section>
  )
...
export default function BrandingPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-stone-500 dark:text-stone-400">{t('dashboard.branding.subtitle')}</p>
      {error && (
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        <div className="flex flex-col gap-5">
          <Section icon={ImageIcon} title={t('dashboard.branding.logo')}>
            <div className="flex items-center gap-3">
              {business?.logo_url ? (
                ...
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-stone-100 text-stone-400 dark:bg-stone-800 dark:text-stone-500">
                  <ImageIcon size={20} />
                </div>
              )}
              <input
              ...
              >
                {uploadingLogo ? t('dashboard.branding.uploading') : t('dashboard.branding.uploadLogo')}
              </Button>
            </div>
            ...
            <div className="mt-5 border-t border-stone-100 pt-5 dark:border-stone-800">
              ...
              <div className="flex items-center gap-3">
                {business?.cover_url ? (
                  ...
                  <div className="flex h-16 w-28 items-center justify-center rounded-lg bg-stone-100 text-stone-400 dark:bg-stone-800 dark:text-stone-500">
                    <ImageIcon size={20} />
                  </div>
                )}
                <input
                ...
                >
                  {uploadingCover ? t('dashboard.branding.uploading') : t('dashboard.branding.uploadCover')}
                </Button>
              </div>
            </div>
          </Section>

          <Section icon={Palette} title={t('onboarding.branding.accent')}>
            <div className="flex flex-wrap gap-2">
              {ACCENT_KEYS.map((key) => (
                <button
                  ...
                  className={`flex h-9 w-9 items-center justify-center rounded-full transition ${ACCENTS[key].main} ${
                    accentKey === key ? 'ring-2 ring-offset-2 ring-stone-400 dark:ring-offset-stone-900' : ''
                  }`}
                  aria-label={key}
                >
          ...
          <Section icon={Type} title={t('onboarding.branding.tagline')}>
            <input
              value={tagline}
              onChange={(e) => setTagline(e.target.value)}
              placeholder={t('onboarding.branding.taglinePlaceholder')}
              className="w-full rounded-xl border border-stone-200 px-4 py-2.5 outline-none focus:border-stone-400 focus:ring-2 focus:ring-stone-200 dark:border-stone-700 dark:bg-stone-900 dark:text-stone-100 dark:focus:border-stone-500 dark:focus:ring-stone-700"
            />
        ...
        <div className="flex flex-col gap-5">
          <Section icon={Images} title={t('dashboard.branding.gallery')}>
            <p className="mb-3 text-xs text-stone-400 dark:text-stone-500">{t('dashboard.branding.galleryHint')}</p>
              ...
              {(business?.gallery_urls ?? []).length < GALLERY_MAX_IMAGES && (
                <>
                  ...
                  <button
                    ...
                    className="flex h-20 w-28 items-center justify-center rounded-lg border border-dashed border-stone-300 text-xs font-medium text-stone-500 hover:border-stone-400 dark:border-stone-700 dark:text-stone-400 dark:hover:border-stone-600"
                  >
                    {uploadingGallery ? t('dashboard.branding.uploading') : t('dashboard.branding.addPhoto')}
                  </button>
                </>
              )}
            </div>
          ...
          <Section icon={ImageIcon} title={t('dashboard.branding.preview')}>
            <div className="mx-auto max-w-sm overflow-hidden rounded-2xl border border-stone-200 shadow-sm dark:border-stone-700 dark:shadow-none">
              <div
              ...
              <div className={`p-4 text-center ${getAccent(accentKey).canvas}`}>
                <p className="font-display text-base font-semibold text-stone-900 dark:text-stone-50">
                  {business?.name}
                </p>
                {tagline && <p className="mt-1 text-sm text-stone-500 dark:text-stone-400">{tagline}</p>}
              </div>
            </div>
          </Section>
        </div>
