    ...
    <main className="min-h-screen bg-stone-50 px-6 py-12 dark:bg-stone-950">
      <div className="mx-auto max-w-4xl">
        ...
        <p className="mt-1 text-sm text-stone-500 dark:text-stone-400">
          Every plan is a paid subscription billed monthly in EUR — there's no free tier. Cancel anytime.
        </p>
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-3">
          {PLANS.map((plan) => (
            ...
            >
              ...
              <h2 className={`font-display text-lg font-semibold ${plan.highlighted ? 'text-white' : 'text-stone-900 dark:text-stone-50'}`}>
                {plan.name}
              </h2>
              <p className={`mt-1 text-2xl font-semibold ${plan.highlighted ? 'text-white' : 'text-stone-900 dark:text-stone-50'}`}>
                €{plan.priceEur}
                <span className={`text-sm font-normal ${plan.highlighted ? 'text-stone-300' : 'text-stone-500 dark:text-stone-400'}`}>
                  /mo
                </span>
              </p>
              <p className={`mt-2 text-sm ${plan.highlighted ? 'text-stone-300' : 'text-stone-500 dark:text-stone-400'}`}>
                {plan.staffLine}
              </p>
              <ul className={`mt-3 flex flex-col gap-1.5 text-sm ${plan.highlighted ? 'text-stone-200' : 'text-stone-600 dark:text-stone-300'}`}>
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-center gap-1.5">
                    <Check size={14} className={plan.highlighted ? 'text-amber-400' : 'text-emerald-600 dark:text-emerald-400'} />
                    {feature}
                  </li>
                ))}
              </ul>
